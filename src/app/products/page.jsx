...
export default function ProductsPage() {
  ...
  const { handleUpdate } = useUpdateProduct(token, offset);
  ...
  return (
    <div className="bg-[#1C2321] text-[#EEF1EF]">
      <div className="p-6 min-h-screen  max-w-7xl mx-auto">
        ...
        {!token ? (
          <p className="mt-6 text-[#EEF1EF]">
            Please log in to see the products.
          </p>
        ) : loadingList || loadingCategories ? (
          <Loader />
        ) : errorList || errorCategories ? (
          <p className="mt-6 text-[#EEF1EF]">
            {typeof (errorList || errorCategories) === "string"
              ? errorList || errorCategories
              : (errorList || errorCategories)?.message ||
                JSON.stringify(errorList || errorCategories)}
          </p>
        ) : products.length > 0 ? (
...
//       <p className="text-[#EEF1EF] mb-3 line-clamp-2 flex-1">
//         {product.description}
//       </p>
